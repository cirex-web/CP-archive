...
int main() {
    ...
    double expected = 0;
    FR(_,K){
        double newExpected = 0;
        FR(i,sz(probs))newExpected+=probs[i]*(max(double(i),expected));
        if(int(newExpected)!=int(expected))cout<<newExpected<<endl;
        swap(expected,newExpected);
    }
    cout<<setprecision(15)<<fixed<<1+expected<<endl;
    return 0;
