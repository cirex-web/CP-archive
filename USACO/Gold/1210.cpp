...
int main() {
    ...
    double ans = 0;
    FRE(take,N+1){ //num of probabilities to take
        double probMoveOn = 0;
        FR(i,N+1-take)probMoveOn+=probs[i];
        double pFac = (1-power(probMoveOn,K))/(1-probMoveOn);
        double expected = 0;
        ROF(i,N,N-take+1){
            expected+=probs[i]*pFac*i;
        }
        double probForLast = power(probMoveOn,K-1);
        FR(i,N+1-take)expected+=probForLast*probs[i]*i;
        setmax(ans,expected);
//        cout<<ans<<" "<<expected<<endl;
